...
namespace MathTools.Algebra.Functions.Tests
{
    ...
    public class MinTests
    {
        ...
        public void EvalTest()
        {
            ...
            var formula = Formula.Parse("Min(3.4,2.0)/3.8");
            Assert.AreEqual(Math.Min(3.4, 2.0) / 3.8, formula.Eval(), error);

            formula = Formula.Parse("3.4/Min(3.8+1.9,5.0)");
            Assert.AreEqual(3.4 / Math.Min(3.8 + 1.9, 5.0), formula.Eval(), error);
        }
        ...
        public void EvalDerivativeTest()
        {
            ...
            {
                var formula = Formula.Parse("x^4*2/4*Min(x,x*2)");
        ...
        public void SimplifyTest()
        {
            ...
            var formula = Formula.Parse("10+Min(3.4,3.0)*3.8+20");
            Assert.AreEqual(formula.Eval(), formula.Simplify().Eval(), error);

            formula = Formula.Parse("2+3.4/Min(3.8+1.9,5.0)*20");
            Assert.AreEqual(formula.Eval(), formula.Simplify().Eval(), error);
        }
        ...
        public void GetDifferentialExpressionTest()
        {
            ...
            void check(double x)
            {
                var formula = Formula.Parse("x^4*Min(x,Exp(x))+x/20*x^2");
                var value = (Math.Pow(x, 4.0) * Math.Min(x, Math.Exp(x))) + (x / 20.0 * Math.Pow(x, 2.0));
