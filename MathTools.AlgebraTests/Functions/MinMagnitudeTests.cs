    ...
    public class MinMagnitudeTests
    {
        ...
        public class MaxTests
        {
            ...
            public void EvalTest()
            {
                ...
                var formula = Formula.Parse("MinMagnitude(-3.4,2.0)/3.8");
                Assert.AreEqual(Math.MinMagnitude(-3.4, 2.0) / 3.8, formula.Eval(), error);

                formula = Formula.Parse("3.4/MinMagnitude(-3.8+1.9,5.0)");
                Assert.AreEqual(3.4 / Math.MinMagnitude(-3.8 + 1.9, 5.0), formula.Eval(), error);
            }
            ...
            public void EvalDerivativeTest()
            {
                ...
                {
                    var formula = Formula.Parse("x^4*2/4*MinMagnitude(-x,x*2)");
                    var x = new Variable("x");
                    var dif = Formula.If(Formula.Abs(x * 2)-Formula.Abs(-x)  ,
            ...
            public void SimplifyTest()
            {
                ...
                var formula = Formula.Parse("10+MinMagnitude(3.4,3.0)*3.8+20");
                Assert.AreEqual(formula.Eval(), formula.Simplify().Eval(), error);

                formula = Formula.Parse("2+3.4/MinMagnitude(3.8+1.9,5.0)*20");
                Assert.AreEqual(formula.Eval(), formula.Simplify().Eval(), error);
            }
            ...
            public void GetDifferentialExpressionTest()
            {
                ...
                void check(double x)
                {
                    var formula = Formula.Parse("x^4*MinMagnitude(-x,Exp(x))");
                    var value = (Math.Pow(x, 4.0) * Math.MinMagnitude(-x, Math.Exp(x)));
