...
namespace MathTools.Algebra.Functions.Tests
{
    ...
    public class AtanhTests
    {
        ...
        public void EvalTest()
        {
            ...
            var formula = Formula.Parse("atanh(0.4)/3.8");
            Assert.AreEqual(Math.Atanh(0.4) / 3.8, formula.Eval(), error);

            formula = Formula.Parse("3.4/atanh(0.8)");
            Assert.AreEqual(3.4 / Math.Atanh(0.8), formula.Eval(), error);
        }
        ...
        public void EvalDerivativeTest()
        {
            ...
            formula = Formula.Parse("x^4*atanh(x)");
            var x = 0.2;
            var vars = new Dictionary<string, double> { { "x", x } };

            Assert.AreEqual(Math.Pow(x,3)*(x/(1-Math.Pow(x,2))+4*Math.Atanh(x)), formula.EvalDerivative("x", vars), error);
        }
        ...
        public void GetDifferentialExpressionTest()
        {
            ...
            var formula = Formula.Parse("x^4*atanh(x)");
